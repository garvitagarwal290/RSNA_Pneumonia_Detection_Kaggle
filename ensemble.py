...
def GeneralEnsemble(dets, iou_thresh = 0.1, singlebox_scorethresh = 0.1, weights=None):
    ...
    for idet in range(0,ndets):
        ...
        for box in det:
            ...
            #if len(found) == 0 and box[5]>singlebox_scorethresh:
             #   new_box = list(box)
              #  new_box[5] *= weights[idet]
               # out.append(new_box)
            #else:
            if((len(found) == 0 and box[5]>singlebox_scorethresh) or len(found)!=0):
                ...
                out.append(new_box)
    return [annot[:4] for annot in out], [annot[5] for annot in out], [annot[4] for annot in out] 
